...
ISBNを入力するテキストボックス, 書籍を検索するボタン, 検索結果を表示するためのdiv要素, 本棚を表示するul要素を記述する.  
...
上記のコードの処理の流れは以下の通りである.

1. `fetch`メソッドを使用し, HTTPリクエストを送信する.
2. 送信したリクエストにレスポンスが返却されると, `then`コールバックが呼び出される.
3. リクエストが成功したかどうかを, `ok`プロパティを使用し確認する.  
`ok`プロパティは, HTTPステータスコードが200番台であれば`true`を返し、400や500番台であれば`false`を返す.  
4. `json`メソッドを使用し, HTTPレスポンスをJSONとしてパースする.
5. HTTP通信にはエラーがつきものであるため, else文と`catch`メソッドを使用し, エラーをコンソールに表示する.

コンソールを見ると以下の画像の様に, 書籍のデータが返ってくる.
...
まず, 書籍を検索する機能を作成する.  
先ほどのFetch APIを使用する.  
また, テキストボックスに値を入れていない場合, `入力してください。`とコンソールに表示されるようにする.
