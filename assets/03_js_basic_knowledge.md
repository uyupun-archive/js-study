...
変数は半角アルファベット(`a`〜`z`, `A`〜`Z`), 数字(`0`〜`9`)アンダースコア(`_`), ダラー(`$`)を組み合わせた文字を使用できる.
...
データ型は**プリミティブ型**と**オブジェクト**の２首類に大別される.
...
以下に挙げる４種類のデータ型に関しては, リテラル表現が用意されている.
- 真偽値
- 数値
- 文字列
- null
...
例えば代入を表現する`=`演算子や, 加算を表現する`+`演算子などである.
...
上の例ではオペランドを２つ取っているため, `=`演算子はオペランドが２つということになる.
